...
*- ntuple declaration
*
      COMMON /KINE/ ip,icode(maxp)	    	     ! icode - particle code
     & ,px(maxp),py(maxp),pz(maxp),E(maxp),m(maxp)   ! px-z - momentum, E=Energy, m=mass
     & ,Ptot(maxp),PT(maxp),fin(maxp),fhit(maxp)     ! Ptot - momentum of particle	
     & ,Rhit(maxp),dthit(maxp),thB(maxp),iz(maxp)    ! PT - transversal momentum of particle
     & ,ihit(maxp), Eta(maxp)                        ! fin - phi from generator, fhit - phi with B-field effect
                                                ! Rhit - partilce impact point with B-field effect
                                                ! dthit - difference of time of flight for "B-field" and "stright" particle
                                                ! iz - the side indicator Left or Right
                                                ! ihit - indicate the "slowest" particle i.e. with max. dthit
      INTEGER ie,Ntot,Nch,Nn,NhitL,NhitP
      INTEGER nh,nhl,nhr
      REAL ET
      COMMON /EVENT/ ie,Ntot,Nch,Nn,NhitP,NhitL ! Ntot - total number particles in event
     & ,nh,nhl,nhr,ET				! Nch - number of charged particles in event
                                                ! Nn - number of neutral particles in event
                                                ! NhitL - number of particles in "reference" side (-3<eta<0)
                                                ! NhitP - number of particles in "signal" side (0<eta<3)   
                                                ! nh - total number of accepted hits in accepted events
                                                ! nhl- number of accepted hits in accepted events - left side
                                                ! nhr- number of accepted hits in accepted events - right side
...
*
      ...
      CALL HLIMIT(Lpawc)
      CALL HCDIR('//PAWC',' ')
      CALL HROPEN(99,'CLBR','lpair.hbook','N',1024,IER)
